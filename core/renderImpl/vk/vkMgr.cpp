...
namespace XD::Render::Vk::VkMgr
{
    ...
    void checkVkResultCType(VkResult r, std::string errInfo)
    {
        if (r == 0) return;
        throw Exce(__LINE__, __FILE__, ("XD::VkMgr Exce: Result = " + std::to_string((int)r) + "\n" + errInfo).c_str());
        if (r < 0) abort();
    }

    void checkVkResult(vk::Result r, std::string errInfo)
    {
