...
namespace XD::Render
{
    ...
    public:
        ...
        /// @param order 排序方式
        BufferLayoutBase(std::initializer_list<Format> types, BufferLayoutRate rate = BufferLayoutRate::Vertex);

        /// @brief 创建缓冲布局
        /// @param types 单元内部的类型 (location 为 0~types.size() 顺序排列)
        /// @param rate 设置缓冲单元按实例输入还是按顶点输入
        /// @param order 排序方式
        BufferLayoutBase(std::vector<Format> types, BufferLayoutRate rate = BufferLayoutRate::Vertex);
...
#undef BufferLayoutTypeMapDef
    ...
    private:
        ...
        { return std::vector<Format>{(Format)Vals...}; }
    ...
    public:
        ...
        using _xd_offset_list = decltype(calcOffsetImpl<0>(std::integer_sequence<size_t>{}, _xd_format_list{}));
        ...
        static auto getInitList()
        { return getInitListImpl(std::integer_sequence<uint8_t, (uint8_t)layouts...>{}); }
    };
}
